				...
				switch(c)
				{
					...
					case 'u':
						...
						index +=4;
							...
							result += UnicodeCodePointToUtf8(code_point, json_string, index);
...
std::string JsonParser::UnicodeCodePointToUtf8(int code_point, const std::string& json_string, size_t index)
{
	if (code_point < 0 || code_point > 0x10FFFF) 
	{
		ThrowError("Invalid Unicode code point", json_string, index);
  	}
  	std::string result;
  	if (code_point <= 0x7F) {
    // One-byte encoding.
...
void JsonParser::ThrowError(const std::string& message, const std::string& json_string, size_t index)
{
    ...
    throw JsonParseException(message + " at line " + std::to_string(line) + ", column " + std::to_string(column));
}
