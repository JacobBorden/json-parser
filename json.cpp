...
double JsonParser::ParseNumber(const std::string& json_string, size_t& index)
{
	...
	if(index < json_string.length() && (json_string[index] == 'e' || json_string[index] == 'E'))
	{
		...
		{
			throw JsonParseException("Expected digit after exponent");
		}
		...
		if(exponent_negative)
			exponent_value = -(double)exponent;
		else exponent_value = (double)exponent;
...
void JsonParser::SkipComment(const std::string& json_string, size_t& index)
{
	...
	{
		char c = json_string[index];
		if(index + 1 < json_string.length() && c == '/' && json_string[index +1] == '/')
		{
			while (index < json_string.length() && json_string[index] != '\n')
					++index;
		}
		else if (index + 1 < json_string.length() && c == '/' && json_string[index+1] =='*')
		{
			index +=2;
			while (index < json_string.length() && !(index + 1 < json_string.length() && json_string[index] == '*' && json_string[index +1] == '/'))
				++index;
			index +=2;
		}
		else break;
